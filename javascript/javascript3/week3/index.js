...
const BASE_URL_SCREENSHOT_API = 'https://website-screenshot6.p.rapidapi.com/screenshot/';
...
refs.libraryScreenshotsBtn.addEventListener('click', onLibraryScreenshotsBtnClick);
...
function onMakeScreenshotBtnClick() {
    ...
    fetchScreenshotApi(refs.input.value).then(data => {
...
function onSaveScreenshotBtnClick() {
    ...
    console.log(screenshotUrl);
    fetchSavingScreenshot(screenshotUrl).then(data=> console.log(data)).catch(error=>console.log(error))

}

function onLibraryScreenshotsBtnClick() {

    fetchLibraryScreenshots().then(data => {
        refs.libraryList.innerHTML = "";
...
function onDeleteBtnClick(event) {
    if (event.target.classList.contains('delete-btn')) {
        ...
        setTimeout(()=>{ fetchLibraryScreenshots().then(data => createLibraryMarkup(data))
        .catch(error=>console.log(error))
    }, 2000)
...
async function fetchScreenshotApi(website) {
    ...
    const response = await fetch(`${BASE_URL_SCREENSHOT_API}?RapidAPI =${KEY_API_SCREENSHOT}&url=https://${website}&width=400&height=600&fullscreen=false&Request=rapidapi.com`, options);
...
console.log(savedUrl)
    ...
    const response = await fetch(`${BASE_URL_CRUD_CRUD}/${KEY_API_CRUD_CRUD}/library`, options);
    const data = await response.json();
    return data
}
...
async function fetchDeleteScreenshot(id) {
    ...
    const response = await fetch(`${BASE_URL_CRUD_CRUD}/${KEY_API_CRUD_CRUD}/library/${id}`, options);
