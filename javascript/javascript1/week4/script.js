...
function getReply(command) {
  ...
  const commandTransformToArray = stringWithoutSpecChars
    .toLowerCase()
    .split(" ");
  const arrayWithoutDuplicatedWords = commandTransformToArray.filter(
    (word, index, array) => array.indexOf(word) === index
  ); //    make new array without dabbled words
  ...
  if (
    arrayWithoutDuplicatedWords.includes("name") &&
    arrayWithoutDuplicatedWords[0] === "hello"
  ) {
    return setName(arrayWithoutDuplicatedWords);
  }

  if (
    arrayWithoutDuplicatedWords.includes("what") &&
    arrayWithoutDuplicatedWords.includes("name")
  ) {
  ...
  if (
    arrayWithoutDuplicatedWords[0] === "add" &&
    arrayWithoutDuplicatedWords.includes("todo")
  ) {
    return addTodo(todo);
  }

  if (
    arrayWithoutDuplicatedWords.includes("remove") &&
    arrayWithoutDuplicatedWords.includes("todo")
  ) {
  ...
  if (
    arrayWithoutDuplicatedWords.includes("what is on my") &&
    arrayWithoutDuplicatedWords.includes("todo")
  ) {
    return getTodo();
  }
  ...
  if (
    arrayWithoutDuplicatedWords.includes("what") &&
    arrayWithoutDuplicatedWords.includes("day") &&
    arrayWithoutDuplicatedWords.includes("today")
  ) {
    return getDate();
  }
  ...
  if (
    command.toLowerCase().startsWith("set a timer for") &&
    command.toLowerCase().endsWith("minutes") &&
    /\d/.test(command)
  ) {
...
console.log(getReply("what fishing from my todo"));
